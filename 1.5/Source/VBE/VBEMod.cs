...
public class VBEMod : Mod
{
    ...
    public void InitializeAnimated()
    {
        ...
        if (initing)
        {
            animatedQueue.EnqueueRange(DefDatabase<BackgroundImageDef>.AllDefs.Except(animatedQueue).Where(def => def.NeedsInit));
            return;
        }

        animatedQueue = new Queue<BackgroundImageDef>();
        animatedQueue.EnqueueRange(DefDatabase<BackgroundImageDef>.AllDefs.Where(def => def.NeedsInit));
    ...
    public override void DoSettingsWindowContents(Rect inRect)
    {
        ...
        if (ModCompat.RimThemes)
            listing.CheckboxLabeled("VBE.RimThemes".Translate(), ref Settings.rimThemesOverride, "VBE.RimThemes.Desc".Translate());
