...
{
    public static AccessTools.FieldRef<UI_BackgroundMain, Dictionary<ExpansionDef, float>> expansionImageFades =
        AccessTools.FieldRefAccess<UI_BackgroundMain, Dictionary<ExpansionDef, float>>("expansionImageFades");
    ...
    public static Rect GetBGRect(Texture texture)
    {
        ...
        if (screenWidth > screenHeight * (vector.x / vector.y))
        {
            ...
            rect = new(0f, (screenHeight - num2) * 0.5f, width, num2);
        }
        else
        {
            float height = screenHeight;
            var num = screenHeight * (vector.x / vector.y);
            rect = new((UI.screenWidth - num) * 0.5f, 0f, num, height);
        }
    ...
    [DebugAction("General", "[VBE] Log Controller State", allowedGameStates = AllowedGameStates.Entry)]
